   ...
   // /user/login: /user/register  - post请求: - 参数: - username: 用户名 - password: 密码 验证码 ：code
    login() {   //$refs 获取到dom元素  上面ref绑定了ruleForm  也可以通过传一个实参 这里传形参
      ...
      axios.post('/api/user/login',{
        ...
        localStorage.setItem('date',this.$dayjs(Date))                //存储登录时间
        sessionStorage.setItem('username',this.ruleForm.username) //短期存储用户名
        sessionStorage.setItem('code',this.ruleForm.code)         //短期存储验证码
