      ...
      <div class="fl">
        <div>
          ...
          >
            ...
            <el-table-column property="time" label="发布时间" width="300"></el-table-column>
...
export default {
  ...
  methods: {
     time(){
       let date =dayjs(this.Article).format('YYYY年MM月DD日HH时mm分ss秒')
    },
    ...
    handleEdit(row) {
      axios.post('/api/article/update',{
        ...
        abstract:row.abstract,        author:row.author,
        category:row.category,        source:row.source,
        star:row.star,  text:row.text, date:row.date
      }).then(res =>{
    ...
    //  - 查看单个文章: /article/article  - post请求  - 参数:  _id: 文章的_id
    handleClick(row) {   //这里row是形参 就是文章的_id
      axios.post('/api/article/article',{_id:row}).then(res =>{
        // console.log(res.data.data);
        this.edit =res.data.data  //赋值
        this.$router.push({name:'check',query:{edit:this.edit}})
      })
    }
  },
  mounted() {
