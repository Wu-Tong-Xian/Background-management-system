<template>
 <div  class="btn-box fl j-between">
  ...
  <!--todolist 头部  -->
      ...
      <div v-for="(item,index) in tolist " :key='index' class="">
      ...
      <div class="fl  j-around mg-t20 to-bottom c-pointer">
        <div class="to-bottom-text">{{tolist.checked.length}} items left</div>
...
<script>
 ...
 export default {
   ...
   methods: {
    ...
    // 显示所有
    all(){
      this.tolist=this.todolist.filter(item=>{
        return item
      })
    ...
    // 未完成
    active(){
     this.tolist=this.tolist.filter(item =>{
